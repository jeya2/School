/* ============================================================
   server/demo.js — the sample school

   A fresh deployment starts empty. This module builds one complete,
   self-consistent school so that an operator can see every screen
   working before a real school's file is imported, and so the test
   suite has a known dataset to assert against.

   It used to live in core.js and run in the browser. It runs here now
   because the database is the source of truth: the demo school is
   imported through exactly the same path as a real school's data file,
   with no privileged route into the app.

   Everything is generated from a fixed seed, so the same records come
   out every time — tests depend on specific ones, notably S4102.

   The defects seeded below are deliberate. Real student masters are
   never clean, and the data-quality engine needs true faults to find.
   Do not "fix" them.
   ============================================================ */
const {
  CLASSES, SECTIONS, COMMUNITIES, RELIGIONS, MEDIUMS, GROUPS, BLOOD, OCCUPATIONS,
  FEE_BY_CLASS, subjectsFor, rng, WORKING_DAYS
} = require('../assets/js/domain.js');
...
/* Staff accounts the sample school signs in with. Passwords are only
   defaults for a demo import — provisionUsers() in server/auth.js hashes
   them like any other account, and the operator is told to change them. */
const DEMO_ACCOUNTS = [
  { username: 'admin',      name: 'R. Saravanan',      title: 'Office Superintendent', role: 'admin',      password: 'demo' },
  { username: 'principal',  name: 'L. Priyadharshini', title: 'Principal',             role: 'principal',  password: 'demo' },
  { username: 'kavitha',    name: 'M. Kavitha',        title: 'Class Teacher · X-A',   role: 'teacher',    password: 'demo' },
  { username: 'accounts',   name: 'P. Elangovan',      title: 'Accountant',            role: 'accountant', password: 'demo' },
  { username: 'parent4102', name: 'S. Murugesan',      title: 'Parent of Karthik Raja',role: 'parent',     password: 'demo', sid: 'S4102' },
  { username: 'stu4102',    name: 'Karthik Raja S',    title: 'Std X-A · Roll 12',     role: 'student',    password: 'demo', sid: 'S4102' }
];
...
/* ---------- Public entry point ---------- */
/** Build the complete sample school as an import bundle — the same shape
 *  a real school supplies in its data file. */
function generate() {
  const students = buildStudents();
  const attHistory = buildAttHistory(students);   // also fills attPresent/attTotal
  return {
    school: DEMO_SCHOOL,
    students,
    attHistory,
    attDays: WORKING_DAYS,
    marks: buildMarks(students),
    receipts: buildReceipts(students),
    notices: NOTICES,
    staff: STAFF,
    attendance: {},
    applications: []
...
module.exports = { generate, DEMO_SCHOOL, DEMO_ACCOUNTS };
